...
const LANG = {
  vi: {
    'DASHBOARD':'BẢNG ĐIỀU KHIỂN', 'Home':'Trang chủ',
    'ENERGY':'NĂNG LƯỢNG', 'Production & Flow':'Sản lượng & Dòng điện',
    'SMART PLUGS':'Ổ CẮM THÔNG MINH', 'Load & Switches':'Tải & Công tắc',
    'BATTERY':'PIN LƯU TRỮ', 'Status & Settings':'Trạng thái & Cài đặt',
    'CLIMATE':'KHÍ HẬU', 'Temperature & Humidity':'Nhiệt độ & Độ ẩm',
    'SECURITY':'AN NINH', 'Alarms & Cameras':'Báo động & Camera',
    'AUTOMATION':'TỰ ĐỘNG HÓA', 'Scenes & Routines':'Kịch bản & Tự động',
    'LIGHTING':'CHIẾU SÁNG', 'Lights & Ambience':'Ánh sáng & Không gian',
    'SYSTEM':'HỆ THỐNG', 'System & Preferences':'Hệ thống & Tùy chọn',
    "TODAY'S CONSUMPTION":'TIÊU THỤ HÔM NAY', "TODAY'S PRODUCTION":'SẢN LƯỢNG HÔM NAY',
    'RECENT EVENTS':'SỰ KIỆN GẦN ĐÂY', 'INV LOAD':'TẢI BIẾN TẦN', 'No Errors':'Không có lỗi',
    'IDLE':'CHỜ', 'DISCHARGING':'ĐANG XẢ', 'CHARGING':'ĐANG SẠC',
    'CELL TEMP':'NHIỆT CELL', 'LOAD':'TẢI TIÊU THỤ', 'CELL VOLT':'ÁP CELL', 'BMS TEMP':'NHIỆT BMS',
    'ENDURANCE':'THỜI GIAN CÒN LẠI', 'BATT CURRENT':'DÒNG ĐIỆN PIN', 'CAPACITY':'DUNG LƯỢNG',
    'GRID PHASES':'PHA LƯỚI', 'INVERTER':'BIẾN TẦN',
    'PV1 POWER':'CÔNG SUẤT PV1', 'PV2 POWER':'CÔNG SUẤT PV2', 'PV3 POWER':'CÔNG SUẤT PV3', 'PV4 POWER':'CÔNG SUẤT PV4',
    'PV5 POWER':'CÔNG SUẤT PV5', 'PV6 POWER':'CÔNG SUẤT PV6',
    'PV TOTAL':'TỔNG CÔNG SUẤT PV', 'PV1 VOLT':'ĐIỆN ÁP PV1', 'PV2 VOLT':'ĐIỆN ÁP PV2', 'PV3 VOLT':'ĐIỆN ÁP PV3', 'PV4 VOLT':'ĐIỆN ÁP PV4',
    'PV5 VOLT':'ĐIỆN ÁP PV5', 'PV6 VOLT':'ĐIỆN ÁP PV6',
    'WEATHER':'THỜI TIẾT', 'TEMPERATURE':'NHIỆT ĐỘ', 'WIND SPEED':'TỐC ĐỘ GIÓ', 'WIND DIR':'HƯỚNG GIÓ', 'SUN':'MẶT TRỜI',
    'GRID POWER':'CÔNG SUẤT LƯỚI', 'GRID VOLT':'ĐIỆN ÁP LƯỚI', 'GRID IMPORT':'MUA LƯỚI', 'GRID EXPORT':'BÁN LƯỚI',
    'L1 VOLT':'ĐIỆN ÁP L1', 'L2 VOLT':'ĐIỆN ÁP L2', 'L3 VOLT':'ĐIỆN ÁP L3',
    'BATTERY SOC':'SOC PIN', 'BATTERY POWER':'CÔNG SUẤT PIN', 'BATTERY CURRENT':'DÒNG ĐIỆN PIN', 'BATTERY VOLT':'ĐIỆN ÁP PIN',
    'MIN CELL':'CELL THẤP NHẤT', 'MAX CELL':'CELL CAO NHẤT', 'Batt Discharge':'Xả pin',
    'BATT2 POWER':'CÔNG SUẤT PIN 2', 'BATT2 CURRENT':'DÒNG ĐIỆN PIN 2', 'BATT2 VOLT':'ĐIỆN ÁP PIN 2',
    'INVERTER TEMP':'NHIỆT BIẾN TẦN', "TODAY'S PV":'PV HÔM NAY', "TODAY'S LOAD":'TẢI HÔM NAY', 'BATT CHARGE':'SẠC PIN',
    'TOTAL PV':'TỔNG SẢN LƯỢNG PV', 'INV STATE':'TRẠNG THÁI BIẾN TẦN',
    'TOTAL IMP':'TỔNG MUA LƯỚI', 'TOTAL EXP':'TỔNG BÁN LƯỚI',
    'CHARGER STATE':'TRẠNG THÁI SẠC', 'CHARGER POWER':'CÔNG SUẤT SẠC', 'CHARGER CURRENT':'DÒNG ĐIỆN SẠC',
    'CAR SOC':'SOC XE XE', 'CHARGE ETA':'ETA SẠC',
  },
  de: {
...
const SL = {
  ...
  r_cyl:[1120,88,140,250], r_stats:[1268,120,206,205], r_mode:[1268,24,206,92],
  r_pvtile:[1120,344,354,42], r_ev:[1120,390,354,42], r_cons:[1120,436,354,104], r_prod:[1120,544,354,104], r_events:[1120,652,354,108],
  pv:[323,575,360,33], pwr:[720,575,355,33],
  stat_cont:[308,612,772,120],
  stat:{ y:622,h:98,w:170,xs:[320,503,686,869] },
  inv_box:[308,746,196,124],
  inv_right:[506,746,574,124],
  donut_c:[548,767,84,100],
  invt:{ y:763,h:98,w:118,xs:[676,808,940] },
  bot:{ y:900,h:68,w:174,xs:[158,350,542,734,926,1118] },
  icons:{ x:1110, y:20 },
...
const NAV_VIEWS = [
  ['cover',      'CỬA CUỐN',   'Điều khiển cửa cuốn',    'home'],
...
class CasaLuna extends HTMLElement {
  ...
  disconnectedCallback() {
    ...
    if (this._fitStageBound) window.removeEventListener('resize', this._fitStageBound);
    if (this._fitStageBound && window.visualViewport) window.visualViewport.removeEventListener('resize', this._fitStageBound);
  ...
  _styles() {
    return `
      ...
      .stage { position:relative; width:100%; max-width:none; margin:0 auto; aspect-ratio:${VB_W}/${VB_H};
      ...
      :host(.cl-tablet-lite) *::after {
        animation:none !important;
        transition:none !important;
      }
      :host(.cl-tablet-lite) .tileSpin,
      :host(.cl-tablet-lite) .tileBulbOn,
      :host(.cl-tablet-lite) .tileSocketOn,
      :host(.cl-tablet-lite) .tileFlameOn,
      :host(.cl-tablet-lite) .tileSnowOn,
      :host(.cl-tablet-lite) .tileWaterOn,
      :host(.cl-tablet-lite) .tileHeatOn,
      :host(.cl-tablet-lite) .tileRgbOn { animation:none !important; filter:none !important; }
      ...
      .navQuickState { color:#a8cae6; }
      .navcovertile { overflow:hidden; }
      .navcovertile .navFrontLayer { display:flex; flex-direction:column; padding:10px 10px 12px; box-sizing:border-box; }
      .mk-cover-minihead { display:flex; align-items:flex-start; justify-content:space-between; gap:10px; min-height:22px; }
      .mk-cover-minihead .mk-cover-title { font-size:15px; font-weight:800; letter-spacing:.05em; color:#dff1ff; }
      .mk-cover-minihead .mk-cover-sub { font-size:11px; color:#9ec4e2; font-weight:650; margin-top:2px; }
      .mk-cover-card { display:flex; flex-direction:column; gap:10px; min-height:0; }
      .mk-cover-card.large { height:100%; }
      .mk-cover-card.panel { padding:4px 0 0; }
      .mk-cover-stagewrap { position:relative; border-radius:20px; overflow:hidden;
      ...
      .mk-cover-card.large .mk-cover-stagewrap { flex:1; min-height:136px; }
      .mk-cover-card.panel .mk-cover-stagewrap { min-height:205px; }
      .mk-cover-badge { position:absolute; left:50%; top:12px; transform:translateX(-50%);
        min-width:86px; max-width:70%; padding:4px 10px; border-radius:8px;
        background:rgba(45,55,72,.92); border:1px solid rgba(255,255,255,.10);
        box-shadow:0 4px 12px rgba(0,0,0,.25); color:#f5faff; font-size:11px; font-weight:800;
        text-align:center; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; z-index:2; }
      .mk-cover-card.panel .mk-cover-badge { top:16px; font-size:14px; min-width:118px; padding:5px 14px; }
      .mk-cover-stage { position:relative; width:min(82%,300px); margin:40px auto 0; height:112px; }
      .mk-cover-card.panel .mk-cover-stage { margin-top:52px; width:min(84%,360px); height:170px; }
      .mk-cover-hood { position:absolute; left:4%; right:4%; top:0; height:16px; border-radius:3px 3px 2px 2px;
        background:linear-gradient(180deg,#ffffff,#e6ebf2 48%,#c9d0d8 100%);
        border:1px solid rgba(190,200,214,.88); box-shadow:0 2px 8px rgba(0,0,0,.22); }
      .mk-cover-hood::after { content:""; position:absolute; left:-2%; right:-2%; bottom:-6px; height:9px;
        border-radius:2px; background:linear-gradient(180deg,#f8fbff,#dfe4ea 55%,#bcc3cb 100%);
        border:1px solid rgba(187,196,208,.88); }
      .mk-cover-rail { position:absolute; top:18px; bottom:0; width:5px; border-radius:8px;
        background:linear-gradient(180deg,#fdfefe,#d7dde6 55%,#c0c6cf 100%);
        border:1px solid rgba(182,192,205,.92); box-shadow:inset 0 1px 0 rgba(255,255,255,.65); }
      .mk-cover-rail.left { left:6%; }
      .mk-cover-rail.right { right:6%; }
      .mk-cover-curtain { position:absolute; left:8%; right:8%; top:18px; height:90px;
        border-radius:0 0 2px 2px; border:1px solid rgba(200,206,214,.95);
        background:
        ...
        transition:height .28s ease, opacity .28s ease; transform-origin:top center; }
      .mk-cover-stage[data-cover-open="100"] .mk-cover-curtain { opacity:.18; }
      .mk-cover-sill { position:absolute; left:8%; right:8%; bottom:2px; height:6px;
        border-radius:0 0 3px 3px; background:linear-gradient(180deg,#f8fbff,#d7dce3);
        border:1px solid rgba(185,194,205,.9); box-shadow:0 1px 3px rgba(0,0,0,.12); }
      .mk-cover-actions { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:10px; }
      .mk-cover-btn { min-height:58px; border-radius:18px; border:1px solid rgba(255,255,255,.14);
        display:flex; flex-direction:column; align-items:center; justify-content:center; gap:4px; cursor:pointer;
        font-weight:800; letter-spacing:.08em; user-select:none;
        box-shadow:inset 0 1px 0 rgba(255,255,255,.12), 0 6px 14px rgba(0,0,0,.12);
        transition:transform .12s ease, box-shadow .12s ease, border-color .12s ease; }
      .mk-cover-btn:hover { transform:translateY(-1px); box-shadow:inset 0 1px 0 rgba(255,255,255,.14), 0 10px 18px rgba(0,0,0,.18); }
      .mk-cover-btn:active { transform:scale(.98); }
      .mk-cover-btn .mk-cover-ico { font-size:24px; line-height:1; }
      .mk-cover-btn .mk-cover-txt { font-size:11px; }
      .mk-cover-card.panel .mk-cover-btn { min-height:74px; }
      .mk-cover-card.panel .mk-cover-btn .mk-cover-ico { font-size:30px; }
      .mk-cover-card.panel .mk-cover-btn .mk-cover-txt { font-size:16px; }
      .mk-cover-btn.open { background:rgba(67,160,71,.15); color:#43d85b; }
      .mk-cover-btn.stop { background:rgba(229,57,53,.16); color:#ff534d; }
      .mk-cover-btn.close { background:rgba(30,136,229,.16); color:#36a0ff; }
      ...
      .pw-mtile.sec-mini { min-height:52px; padding:7px 6px; }
      .pw-mtile.sec-mini .mi { font-size:13px; line-height:1; }
      .pw-mtile.sec-mini .ml { font-size:8px; margin:2px 0 1px; }
      .pw-mtile.sec-mini .mv { font-size:12px; }
      ...
      .pw-cams.sec-cams { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); align-content:start;
        max-height:none; overflow:visible; padding-right:0; }
      .pw-cam.sec-cam { min-height:188px; aspect-ratio:16/9; }
      .pw-cams.sec-cams::-webkit-scrollbar { width:8px; }
      .pw-cams.sec-cams::-webkit-scrollbar-thumb { background:rgba(120,180,255,.38); border-radius:8px; }
      .pw-cams.sec-cams::-webkit-scrollbar-track { background:rgba(255,255,255,.04); border-radius:8px; }
      ...
      :host(.cl-compact-panel) .detail { width:852px; }
      :host(.cl-compact-panel) .detail-inner { padding:14px 18px; }
      :host(.cl-compact-panel) .detail h3 { font-size:18px; }
      :host(.cl-compact-panel) .detail .dsub { font-size:11px; margin-bottom:10px; }
      :host(.cl-compact-panel) .pw-head { margin:10px 0 6px; font-size:10px; }
      :host(.cl-compact-panel) .pw-cams { gap:8px; margin-bottom:10px; }
      :host(.cl-compact-panel) .pw-cam.sec-cam { min-height:154px; }
      :host(.cl-compact-panel) .pw-mtile.sec-mini { min-height:44px; padding:5px 5px; }
      :host(.cl-compact-panel) .pw-mtile.sec-mini .mi { font-size:12px; }
      :host(.cl-compact-panel) .pw-mtile.sec-mini .ml { font-size:7px; }
      :host(.cl-compact-panel) .pw-mtile.sec-mini .mv { font-size:11px; }
  ...
  _build() {
    ...
    <div style="position:absolute;left:0;top:0;width:${VB_W}px;height:140px;pointer-events:none">
      ...
      <div style="position:absolute;left:0;top:0;width:${VB_W}px;text-align:center;pointer-events:none">
        <div style="margin-top:18px;line-height:.92;font-family:'Bahnschrift','Arial Narrow','Franklin Gothic Medium','Segoe UI',Impact,sans-serif;font-size:56px;font-weight:900;
          ...
          text-shadow:none;
          filter:drop-shadow(0 0 20px rgba(255,140,0,0.65)) drop-shadow(0 0 8px rgba(255,215,0,0.85)) drop-shadow(0 0 2px rgba(255,255,255,0.9))">${esc(c.title || 'MK SOLAR')}</div>
        <div id="hSubtitle" style="margin-top:5px;line-height:1;font-family:'Bahnschrift','Arial Narrow','Segoe UI',sans-serif;font-size:11.2px;font-weight:400;
          letter-spacing:4.8px;color:#a8cae6;text-align:center">ENERGY • AUTOMATION • SECURITY</div>
      </div>
    </div>`;
    ...
    let navY = SL.nav.tops[0] + 4;
    let navBottomY = navY;
    const nav = NAV_VIEWS.map(([key, t, s, ik], i) => {
      const isCamPreview = key === 'cam17preview';
      const isCoverPreview = key === 'cover';
      const navH = isCamPreview ? SL.nav.w : isCoverPreview ? 232 : Math.round(SL.nav.h * 0.9);
      const y = navY;
      navY += navH + 8;
      navBottomY = y + navH;
      const g = NAV_GLOW[i];
      const txtL = 68;
      if (isCamPreview) {
        const camEnt = c.nav_cam_entity || c.sec_cam1 || '';
        return `<div class="box tap navtile navcamtile" data-view="security" data-i="${i}" data-cam-tap="${esc(camEnt)}" data-cam-label="Cam 17"
          style="left:${SL.nav.x}px;top:${y}px;width:${SL.nav.w}px;height:${navH}px;background:rgba(0,0,0,.55);
          box-shadow:${glowShadow(g)};overflow:hidden">
          <img class="camStream navCamStream" data-cam-id="${esc(camEnt)}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;background:#000">
          <div style="position:absolute;inset:0;background:linear-gradient(to bottom,rgba(0,0,0,.35),rgba(0,0,0,.08) 45%,rgba(0,0,0,.55));pointer-events:none"></div>
          <div class="val" style="position:absolute;left:12px;top:10px;font-size:16px;font-weight:800;color:#eaf4ff;text-shadow:0 1px 4px #000">CAM 17</div>
          <div style="position:absolute;left:12px;bottom:10px;right:12px;font-size:12px;font-weight:650;color:#a8cae6;text-shadow:0 1px 4px #000;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">Camera trực tiếp</div>
        </div>`;
      }
      if (key === 'cover') {
        const coverName = c.nav_cover_name || 'Cửa cuốn';
        const coverState = this._coverStateText();
        return `<div class="box tap navtile collapsible" data-view="${key}" data-i="${i}"
          ...
          <div class="navFrontLayer">
            ...
            <div style="position:absolute;left:${txtL}px;top:${navH/2-19}px;right:6px;overflow:hidden;
              ...
              font-family:'Segoe UI',Roboto,'Helvetica Neue',system-ui,sans-serif;white-space:nowrap;text-overflow:ellipsis">${esc(coverName).toUpperCase()}</div>
            <div id="navCoverState" class="navQuickState" style="position:absolute;left:${txtL}px;top:${navH/2+2}px;right:6px;overflow:hidden;
              font-size:12px;font-weight:650;font-family:'Segoe UI',Roboto,'Helvetica Neue',system-ui,sans-serif;
              white-space:nowrap;text-overflow:ellipsis">${esc(coverState)}</div>
          </div>
        </div>`;
      }
      return `<div class="box tap navtile collapsible" data-view="${key}" data-i="${i}"
    ...
    const statDefs = [
      ...
      ['gexp', c.label_daily_total_energy || c.label_grid_export_energy || 'GRID EXP', c.daily_total_energy ? 'home' : 'bolt'],
    ];
    ...
    const prodBox = ([x, y, w, h]) => {
      ...
      const front = `<div class="val" style="position:absolute;left:14px;top:10px;font-size:14px">${c.label_today_production || this._t("TODAY'S PRODUCTION")}</div>
        ...
        <svg id="prChart" viewBox="0 0 ${w - 24} ${chartH}" preserveAspectRatio="none" style="position:absolute;left:12px;top:32px;width:${w - 24}px;height:${chartH}px">
      ...
      return `<div class="box collapsible" data-chart-detail="prod" data-collapse-id="prod" style="left:${x}px;top:${y}px;width:${w}px;height:${h}px;overflow:hidden;background:var(--cl-box-bg,rgba(0,0,0,.35));cursor:pointer">
    ...
    const consBox = ([x, y, w, h]) => {
      const col = '#46bcff';
      const chartH = h - 38;
      const front = `<div class="val" style="position:absolute;left:14px;top:10px;font-size:14px">${c.label_monthly_consumption || 'TI\u00caU TH\u1ee4 TH\u00c1NG N\u00c0Y'}</div>
        ...
        <div class="well" style="position:absolute;left:12px;top:32px;width:${w-24}px;height:${chartH}px"></div>
        <svg id="cnChart" viewBox="0 0 ${w - 24} ${chartH}" preserveAspectRatio="none" style="position:absolute;left:12px;top:32px;width:${w-24}px;height:${chartH}px">
          <defs><linearGradient id="cnFillGrad" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stop-color="${col}" stop-opacity="0.55"/>
            <stop offset="100%" stop-color="${col}" stop-opacity="0"/></linearGradient></defs>
          <path class="fill" fill="url(#cnFillGrad)" stroke="none"/>
          <path fill="none" stroke="${col}" stroke-width="2"/></svg>`;
      return `<div class="box collapsible" data-chart-detail="cons" data-collapse-id="cons" style="left:${x}px;top:${y}px;width:${w}px;height:${h}px;overflow:hidden;background:var(--cl-box-bg,rgba(0,0,0,.35));cursor:pointer">
    ...
    */
    ...
    this._fitStageBound = () => this._fitStageToViewport();
    this._ro = new ResizeObserver(() => this._fitStageToViewport());
    this._ro.observe(this);
    window.addEventListener('resize', this._fitStageBound, { passive: true });
    if (window.visualViewport) window.visualViewport.addEventListener('resize', this._fitStageBound, { passive: true });
    this._fitStageToViewport();
  ...
  _bindEvents() {
    ...
    if (this._navCompact === undefined) this._setNavCompact(true);
    else this._setNavCompact(this._navCompact);
    ...
    /* energy charts: tap the chart card to open a date-aligned data table */
    this.shadowRoot.querySelectorAll('[data-chart-detail]').forEach(el => {
      el.style.cursor = 'pointer';
      el.addEventListener('click', e => {
        if (e.target.closest('.collapseBtn, [data-collapse], [data-expand]')) return;
        e.stopPropagation();
        this._energyDataPopup(el.dataset.chartDetail || 'cons');
      });
    });
  ...
  _wCameras(cams) {
    const cells = cams.map(([label, src]) => {
      const isEntityRef = this._isCameraEntityRef(src);
      const url = !isEntityRef ? this._cameraEmbedUrl(src) : '';
      const body = url
        ? `<iframe src="${esc(url)}" allowfullscreen loading="lazy"></iframe>`
        : src
          ? `<img class="camStream" data-cam-id="${esc(src)}" alt="${esc(label)}" loading="lazy" decoding="async">`
      ...
      return `<div class="pw-cam" ${src ? `data-cam-tap="${esc(src)}" data-cam-src="${isEntityRef ? '' : esc(src)}" data-cam-label="${esc(label)}" data-cam-url="${esc(url)}"` : ''}>
        ${body}
  ...
  _isCameraEntityRef(src) {
    return typeof src === 'string' && /^camera\./i.test(src.trim());
  }

  _wSecurityCameras(cams) {
    const base = this._wCameras(cams);
    return base
      .replace('class="pw-cams"', 'class="pw-cams sec-cams"')
      .replaceAll('class="pw-cam"', 'class="pw-cam sec-cam"');
  }

  _wSecurityCompactGrid(cols, html) {
    return this._wGrid(cols, html).replaceAll('class="pw-mtile"', 'class="pw-mtile sec-mini"');
  }

  _discoverCompactTiles(rules, cols = 4, iconFn = null) {
    ...
    return this._wSecurityCompactGrid(cols, html);
  }

  _cameraIdentityKey(label, src) {
    const raw = `${src || ''} ${label || ''}`;
    const num = raw.match(/(?:cam(?:era)?[_\\s.-]*)(\d{1,3})(?!\d)/i) || raw.match(/\b(\d{1,3})\b/);
    if (num && num[1]) return `cam-${num[1]}`;
  ...
  _securityCameraList() {
    ...
    const addCam = (label, src) => {
      const key = String(src || '').trim();
      const identity = this._cameraIdentityKey(label, key);
      if (!key || seen.has(key) || seenIdentity.has(identity)) return;
      seen.add(key);
      seenIdentity.add(identity);
      list.push([label || this._name(key) || key, key]);
    };
    addCam('Front — Cam 1', c.sec_cam1 || '');
    addCam('Gate — Cam 2', c.sec_cam2 || '');
    this._discover([{ domain: 'camera' }]).forEach(id => addCam(this._name(id), id));
    return list;
  }
  ...
  _bindPanelWidgets(root) {
    ...
    root.querySelectorAll('[data-cam-tap]').forEach(el => el.addEventListener('click', e => {
      ...
      this._openCameraFullscreen(
        el.getAttribute('data-cam-tap'),
        el.getAttribute('data-cam-label') || 'Camera',
        el.getAttribute('data-cam-src') || '',
        el.getAttribute('data-cam-url') || ''
      );
    }));
    this._refreshCameraStreams();
    root.querySelectorAll('[data-coveract]').forEach(el => el.addEventListener('click', e => {
      e.stopPropagation();
      const entityId = el.getAttribute('data-coverid') || this._coverEntityId();
      const act = el.getAttribute('data-coveract');
  ...
  _renderDetail() {
    ...
    this._detailScrollState = this._detailScrollState || {};
    ...
    const prevState = this._detailScrollState[view] || {};
    prevState.innerScroll = inner.scrollTop;
    const existingSecCams = inner.querySelector('.sec-cams');
    if (existingSecCams) prevState.secCamScroll = existingSecCams.scrollTop;
    this._detailScrollState[view] = prevState;
    ...
    inner.scrollTop = prevState.innerScroll || 0;
    const nextSecCams = inner.querySelector('.sec-cams');
    if (nextSecCams) nextSecCams.scrollTop = prevState.secCamScroll || 0;
  }
  ...
  _viewSecurity() {
    const c = this.config;
    const cams = this._securityCameraList();
    /* auto-discover: all cameras + safety binary_sensors */
    if (this._autoOn('security')) {
      return this._wHead('Cameras')
        + this._wSecurityCameras(cams)
        + this._wHead('Safety Sensors (auto)')
        + this._discoverCompactTiles([{ domain: 'binary_sensor', device_class: ['gas', 'smoke', 'carbon_monoxide', 'safety'] }], 4, () => '🔥')
        + this._wHead('Motion & Doors (auto)')
        + this._discoverCompactTiles([{ domain: 'binary_sensor', device_class: ['motion', 'occupancy', 'moving'] }, { domain: 'binary_sensor', device_class: ['door', 'window', 'opening', 'garage_door'] }], 4,
    ...
    return this._wHead('Cameras')
      + this._wSecurityCameras(cams)
      + grp('Safety Sensors', safety ? this._wSecurityCompactGrid(4, safety) : '')
      + grp('Doors & Windows', doors ? this._wSecurityCompactGrid(4, doors) : '')
      + grp('More', extra ? this._wSecurityCompactGrid(4, extra) : '')
  ...
  _viewCover() {
    return this._coverPanelHtml({ mode: 'panel' });
    const c = this.config;
    const id = this._coverEntityId();
    const label = c.nav_cover_name || this._name(id) || 'Cửa cuốn';
    const st = this._coverStateText(id);
    const camId = c.nav_cam_entity || c.sec_cam1 || '';
    const camBlock = camId
      ? this._wHead('Camera') + this._wCameras([[label, camId]])
      : '';
    const controls = `<div class="pw" style="flex-direction:column;align-items:stretch;gap:12px;padding:16px">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px">
        <span class="pw-lbl" style="font-size:15px">${esc(label)}</span>
        <span style="font-size:12px;font-weight:700;color:#7fd4ff">${esc(st)}</span>
      </div>
      <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px">
        <div class="pw-scene" data-coveract="open" data-coverid="${esc(id)}" style="padding:12px 6px;color:#5ae06e">MỞ</div>
        <div class="pw-scene" data-coveract="stop" data-coverid="${esc(id)}" style="padding:12px 6px;color:#ff6a5a">DỪNG</div>
        <div class="pw-scene" data-coveract="close" data-coverid="${esc(id)}" style="padding:12px 6px;color:#55a8ff">ĐÓNG</div>
      </div>
    </div>`;
    return camBlock + this._wHead('Điều khiển') + controls;
  }

  /* ── AUTOMATION view: scenes + relays + automations + Alexa + Tuya timers ── */
  _coverVisualData(entityId = this._coverEntityId()) {
    const st = String(this._st(entityId) || '').toLowerCase();
    let pos = parseFloat(this._attr(entityId, 'current_position'));
    if (!Number.isFinite(pos)) {
      if (st === 'open') pos = 100;
      else if (st === 'closed') pos = 0;
      else if (st === 'opening') pos = 70;
      else if (st === 'closing') pos = 30;
      else pos = 0;
    }
    pos = Math.max(0, Math.min(100, pos));
    const curtainPct = Math.max(0, Math.min(100, 100 - pos));
    const curtainPx = Math.round(10 + curtainPct * 0.96);
    return { state: st, pos, curtainPct, curtainPx };
  }

  _coverPanelHtml({ mode = 'nav' } = {}) {
    const id = this._coverEntityId();
    const label = this.config.nav_cover_name || this._name(id) || 'Cửa cuốn';
    const info = this._coverVisualData(id);
    const badge = this._coverStateText(id);
    const wrapClass = mode === 'panel' ? 'mk-cover-card panel' : 'mk-cover-card large';
    const wrapStyle = mode === 'panel' ? 'max-width:560px;margin:0 auto' : '';
    const headHtml = mode === 'panel'
      ? `<div class="mk-cover-minihead">
        <div>
          <div class="mk-cover-title">${esc(label).toUpperCase()}</div>
          <div class="mk-cover-sub">Äiá»u khiá»ƒn cá»­a cuá»‘n</div>
        </div>
      </div>`
      : '';
    return `<div class="${wrapClass}" style="${wrapStyle}">
      <div class="mk-cover-minihead" style="${mode === 'panel' ? '' : 'display:none'}">
        <div>
          <div class="mk-cover-title">${esc(label).toUpperCase()}</div>
          <div class="mk-cover-sub">Điều khiển cửa cuốn</div>
        </div>
      </div>
      <div class="mk-cover-stagewrap">
        <div class="mk-cover-badge" data-cover-state>${esc(badge)}</div>
        <div class="mk-cover-stage" data-cover-stage data-cover-open="${Math.round(info.pos)}">
          <div class="mk-cover-hood"></div>
          <div class="mk-cover-rail left"></div>
          <div class="mk-cover-rail right"></div>
          <div class="mk-cover-curtain" data-cover-curtain style="height:${info.curtainPx}px"></div>
          <div class="mk-cover-sill"></div>
        </div>
      </div>
      <div class="mk-cover-actions">
        <div class="mk-cover-btn open" data-coveract="open" data-coverid="${esc(id)}">
          <div class="mk-cover-ico">↑</div>
          <div class="mk-cover-txt">MỞ</div>
        </div>
        <div class="mk-cover-btn stop" data-coveract="stop" data-coverid="${esc(id)}">
          <div class="mk-cover-ico">■</div>
          <div class="mk-cover-txt">DỪNG</div>
        </div>
        <div class="mk-cover-btn close" data-coveract="close" data-coverid="${esc(id)}">
          <div class="mk-cover-ico">↓</div>
          <div class="mk-cover-txt">ĐÓNG</div>
        </div>
      </div>
    </div>`;
  }
  ...
  _isTabletViewport() {
    try {
      return !!(window.matchMedia && window.matchMedia('(max-width: 1366px)').matches) || window.innerWidth <= 1366;
    } catch (e) {
      return false;
    }
  }
  _isTouchDevice() {
    try {
      ...
      const touchPts = Number(window.navigator?.maxTouchPoints || 0);
      return coarse || touchPts > 0;
    } catch (e) {
      return false;
    }
  }
  _syncTabletPerformanceMode(enabled) {
    const next = !!enabled;
    if (this._tabletPerfMode === next) return;
    this._tabletPerfMode = next;
    const svgs = this.shadowRoot?.querySelectorAll?.('svg') || [];
    svgs.forEach(svg => {
      try {
        if (next) svg.pauseAnimations && svg.pauseAnimations();
        else if (!document.hidden) svg.unpauseAnimations && svg.unpauseAnimations();
      } catch (e) {}
    });
  }
  _fitStageToViewport() {
    const stage = this._q('.stage');
    const scaler = this._q('#scaler');
    if (!stage || !scaler) return;
    const isMobile = this._isMobileViewport();
    const isTablet = this._isTabletViewport();
    const isTouch = this._isTouchDevice();
    const coarsePointer = !!(window.matchMedia && window.matchMedia('(pointer: coarse)').matches);
    // A Windows PC can report maxTouchPoints > 0 even when the user is using a
    // normal mouse. Treat only mobile/tablet/coarse-pointer screens as a touch
    // layout so desktop Chrome can still stretch to the full available width.
    const touchLayout = (coarsePointer && window.innerWidth <= 1366) || (isTouch && isMobile);
    // HA can constrain a single-card view to a centered ~1120px column. On
    // desktop, explicitly occupy everything from the card's left edge to the
    // right edge of the browser so the dashboard really fills the panel.
    const hostLeft = Math.max(0, this.getBoundingClientRect?.().left || 0);
    const viewportW = Math.max(360, window.visualViewport?.width || window.innerWidth || VB_W);
    const availableHostW = Math.max(360, viewportW - hostLeft);
    this.style.width = `${availableHostW}px`;
    this.style.maxWidth = 'none';
    const hostW = Math.max(360, this.clientWidth || stage.parentElement?.clientWidth || viewportW || VB_W);
    // Do not clamp the viewport to the 920px design height. That kept the card
    // at full height on shorter Chrome windows and created a vertical scrollbar.
    const vpH = Math.max(window.visualViewport?.height || 0, window.innerHeight || 0, 360);
    const top = Math.max(0, this.getBoundingClientRect?.().top || 0);
    const bottomPad = isTablet ? 12 : 18;
    const availH = Math.max(360, vpH - top - bottomPad);
    const widthByHeight = availH * (VB_W / VB_H);
    let fitW = Math.max(360, Math.min(hostW, widthByHeight));
    if (touchLayout) fitW = Math.max(360, Math.min(fitW, VB_W));
    const uniformScale = fitW / VB_W;
    let stageW = fitW;
    let scaleX = uniformScale;
    let scaleY = uniformScale;
    if (!touchLayout && hostW > fitW + 24) {
      stageW = hostW;
      scaleX = hostW / VB_W;
    }
    const stageH = Math.round(VB_H * scaleY);
    const compact = fitW < 1240 || availH < 820;
    const tabletLite = touchLayout && isTablet && !isMobile;
    const fitSig = [Math.round(stageW), stageH, scaleX.toFixed(4), scaleY.toFixed(4), compact ? 1 : 0, tabletLite ? 1 : 0].join('|');
    if (this._lastFitSig !== fitSig) {
      stage.style.maxWidth = touchLayout ? `${VB_W}px` : 'none';
      stage.style.width = `${stageW}px`;
      stage.style.height = `${stageH}px`;
      scaler.style.transform = scaleX === scaleY
        ? `scale(${scaleY})`
        : `scale(${scaleX}, ${scaleY})`;
      this._lastFitSig = fitSig;
    }
    this.classList.toggle('cl-compact-panel', compact);
    this.classList.toggle('cl-tablet-lite', tabletLite);
    this._syncTabletPerformanceMode(tabletLite);
  }
  _cameraEmbedUrl(src, opts = {}) {
    const base = String(this.config.camera_stream_base || '').trim();
    if (!src || !base) return '';
    const mode = this._isMobileViewport() ? 'webrtc' : 'mse';
    const extra = opts.fullscreen ? '' : '&muted=1';
    return `${base}/stream.html?src=${encodeURIComponent(src)}&mode=${mode}${extra}`;
  }
  _mountNavCoverPanel() {
    const tile = this.shadowRoot?.querySelector('.navtile[data-view="cover"]');
    const front = tile?.querySelector('.navFrontLayer');
    if (!tile || !front) return;
    if (front.dataset.coverMounted !== '1') {
      front.innerHTML = this._coverPanelHtml({ mode: 'nav' });
      front.dataset.coverMounted = '1';
      this._bindPanelWidgets(front);
    }
    tile.classList.add('navcovertile');
  }
  _syncCoverWidgets() {
    this._mountNavCoverPanel();
    const info = this._coverVisualData();
    const text = this._coverStateText();
    this.shadowRoot?.querySelectorAll('[data-cover-state]').forEach(el => {
      el.textContent = text;
      el.style.color = info.state === 'open' || info.state === 'opening' ? '#ffcf7c'
        : info.state === 'closed' ? '#f5faff'
        : info.state === 'closing' ? '#9df3b0'
        : '#dfe9f4';
    });
    this.shadowRoot?.querySelectorAll('[data-cover-curtain]').forEach(el => {
      el.style.height = `${info.curtainPx}px`;
      el.style.opacity = info.pos >= 98 ? '.18' : '1';
    });
    this.shadowRoot?.querySelectorAll('[data-cover-stage]').forEach(el => {
      el.setAttribute('data-cover-open', `${Math.round(info.pos)}`);
    });
  }
  _coverEntityId() {
    const preferred = this.config.nav_cover_entity || this.config.cover_main || '';
    if (this._stateObj(preferred)) return preferred;
    return preferred;
  }
  _coverStateText(entityId = this._coverEntityId()) {
    const st = String(this._st(entityId) || '').toLowerCase();
    if (!st || st === 'unknown' || st === 'unavailable') return 'Chưa có trạng thái';
    if (st === 'open') return 'đang mở';
    if (st === 'opening') return 'đang mở lên';
    if (st === 'closed') return 'được đóng';
    if (st === 'closing') return 'đang đóng xuống';
    if (st === 'stopped') return 'đang dừng';
    return this._cap(st);
  }
  ...
  _refreshCameraStreams() {
    ...
    imgs.forEach(img => {
      ...
      if (!id) return;
      const preferStill = this._activeView === 'security' || (img.classList.contains('navCamStream') && this._isTouchDevice());
      if (preferStill) {
        if (img.classList.contains('navCamStream') && img.dataset.touchStillLocked === '1' && img.src) return;
        const pic = this._attr(id, 'entity_picture');
        const stillUrl = pic ? (this._hass?.hassUrl ? this._hass.hassUrl(pic) : pic) : null;
        if (stillUrl && img.src !== stillUrl) img.src = stillUrl;
        if (img.classList.contains('navCamStream') && this._isTouchDevice() && img.src) img.dataset.touchStillLocked = '1';
        return;
      }
      if (img.dataset.touchStillLocked) delete img.dataset.touchStillLocked;
  ...
  _openCameraFullscreen(entityId, label, streamSrc = '', go2rtcUrl = '') {
    ...
    const liveUrl = streamSrc ? this._cameraEmbedUrl(streamSrc, { fullscreen: true }) : go2rtcUrl;
    if (liveUrl) {
      body.innerHTML = `<iframe src="${esc(liveUrl)}" allowfullscreen loading="lazy" style="width:100%;height:100%;border:none"></iframe>`;
    } else {
  ...
  _setNavCompact(folded) {
    ...
    const N = NAV_VIEWS.length;
    const KEEP = Math.max(0, N - 2); // keep main items visible; hide Automation + System by default
    this.shadowRoot.querySelectorAll('.navtile').forEach(t => {
      const i = +t.dataset.i;
      if (!folded || i < KEEP) { t.classList.remove('folded'); t.style.transform = ''; t.style.opacity = ''; return; }
  ...
  _a11yPass(root) {
    ...
    const buttons = r.querySelectorAll('[data-more], [data-press], [data-scene], [data-cam-tap], [data-coveract], .flipbtn, .tap, .navtile, .bottile, .stattile');
    buttons.forEach(el => {
  ...
  _weatherArtFile() {
    ...
    const ext = this.config.use_webp_backgrounds === false ? 'png' : 'webp';
    if (!isDay) return `ve%20dem.${ext}`;
    if (isDawn || isDusk) return `ve%20chieu.${ext}`;
    if (cond === 'rainy' || cond === 'thunderstorm') return `troi%20mua%20bao.${ext}`;
    if (cond === 'cloudy' || cond === 'partlycloudy' || cond === 'fog' || cond === 'snowy') return `troi%20ko%20co%20nang.${ext}`;
    return `troi%20ngay.${ext}`;
  }
  _setBackground(force) {
    const key = this._bgVariantKey();
    const weatherArt = this._weatherArtFile();
    const bgKey = `${key}|${weatherArt}`;
    if (!force && bgKey === this._bgKey) return;
    this._bgKey = bgKey;
    ...
    img.onerror = () => {
      ...
      fallback.onload = () => { showEl.src = fallback.src; showEl.style.opacity = 1; hideEl.style.opacity = 0; };
      fallback.onerror = () => {
  ...
  _update(tick = false) {
    ...
    this._syncCoverWidgets();
    this._setTxt('#navCoverState', this._coverStateText());
    const navCoverState = this._q('#navCoverState');
    if (navCoverState) {
      const st = String(this._st(this._coverEntityId()) || '').toLowerCase();
      navCoverState.style.color = st === 'open' || st === 'opening' ? '#ffb45a'
        : st === 'closed' ? '#7fd4ff'
        : st === 'closing' ? '#5ae06e'
        : '#a8cae6';
    }
    ...
    // GRID IMP / daily total / GRID EXP tiles
    this._setTxt('#v_gimp', this._kwhEnt(c.grid_import_today));
    this._setColor('#v_gimp', '#ffb45a');
    const thirdEnergy = c.daily_total_energy || c.grid_export_energy;
    this._setTxt('#v_gexp', thirdEnergy ? this._kwhEnt(thirdEnergy) : '--');
    ...
    // Battery fill — single or dual (split) cylinder
    if (c._show_battery2) {
      ...
      const applyFill = (socV, barId, hlId, pctId, boltId, powEnt, invKey) => {
        ...
        const bP = (powEnt === c.battery_power) ? this._batteryNetW() : (this._watts(powEnt) * (c[invKey] ? -1 : 1));
      ...
      const bPany = Math.abs(this._batteryNetW()) + Math.abs(this._watts(c.battery2_power));
    ...
    if (c.history_charts && Date.now() >= (this._historyReadyAt || 0)) {
      this._drawPvDailyLine('#prChart', c.today_pv || c.pv_total_power || c.pv1_power || c.pv2_power, SL.r_prod[2] - 24, SL.r_prod[3] - 38);
      this._drawMonthlyBars('#cnChart', c.today_load || c.daily_total_energy || c.consump, SL.r_cons[2] - 24, SL.r_cons[3] - 38, { persistKind: 'cons' });
    }
    ...
    /* detail view refresh + background
       Security view keeps 4 camera streams alive; re-rendering its entire DOM on every
       hass update forces all streams to reconnect and looks jerky on tablets/PCs.
       Freeze the security panel DOM while it is open so only the live streams continue. */
    if (this._activeView !== 'dashboard' && !this._panelBusy) {
      if (this._activeView !== 'security') this._renderDetail();
    }
    this._setBackground();
  }
  ...
  _batteryNetW() {
    const c = this.config || {};
    const configured = this._watts(c.battery_power) * (c.invert_battery_power ? -1 : 1);
    if (Math.abs(configured) > 50) return configured;

    const disW = this._watts(c.battery_discharge_power, NaN);
    const chgW = this._watts(c.battery_charge_power || c.battery_power, NaN);
    if (Number.isFinite(disW) && disW > 50) return disW;
    if (Number.isFinite(chgW) && chgW > 50) return -chgW;

    /* Fallback for LuxPower/BMS setups where the mapped battery_power entity is
       a charge-only value or stale, but the battery current is live.
       In this HA setup negative BMS current means the battery is discharging. */
    const cur = this._num(c.battery_current, NaN);
    const volt = this._num(c.battery_voltage, NaN);
    if (Number.isFinite(cur) && Math.abs(cur) > 0.2) {
      const v = Number.isFinite(volt) && volt > 5 ? volt : 51.2;
      return -cur * v;
    }
    return configured;
  }
  ...
  async _fetchHistory(ent, minutes) {
    ...
    if (cache && Date.now() - cache.t < 15 * 60 * 1000) return cache.pts;
    try {
  ...
  async _fetchHistoryRows(ent, minutes) {
    if (!ent || !this._hass) return null;
    const key = `rows|${ent}|${minutes}`;
    const cache = this._histCache[key];
    if (cache && Date.now() - cache.t < 30 * 60 * 1000) return cache.rows;
    try {
      const start = new Date(Date.now() - minutes * 60000).toISOString();
      const res = await this._hass.callApi('GET',
        `history/period/${start}?filter_entity_id=${ent}&minimal_response&no_attributes`);
      const rows = (res?.[0] || []).map(r => {
        const v = parseFloat(r.state ?? r.s);
        const t = new Date(r.last_changed || r.last_updated || r.lu || r.lc || Date.now()).getTime();
        return Number.isFinite(v) && Number.isFinite(t) ? { t, v } : null;
      }).filter(Boolean);
      this._histCache[key] = { t: Date.now(), rows };
      return rows;
    } catch { return cache?.rows || null; }
  }
  ...
  async _drawPvDailyLine(sel, ent, w, h) {
    return this._drawMonthlyBars(sel, ent, w, h, {
      color: '#7ce05a',
      labelFill: '#eaffde',
      totalSelector: null,
      updateTotal: false,
    ...
    const unit = (this._attr(ent, 'unit_of_measurement') || '').trim().toLowerCase();
    const isEnergy = /wh/.test(unit);
    const factor = isEnergy
      ? (unit === 'wh' ? 1 / 1000 : unit === 'mwh' ? 1000 : 1)
      : (unit === 'kw' ? 1000 : unit === 'mw' ? 1000000 : 1);
    const outUnit = isEnergy ? 'kWh' : 'W';
    let rows = await this._fetchHistoryRows(ent, 24 * 60);
    if (!rows || rows.length < 2) {
      const nowVal = isEnergy
        ? Math.max(0, this._num(ent, NaN) * factor)
        : Math.abs(this._watts(ent, NaN));
      if (!Number.isFinite(nowVal)) return;
      const now = Date.now();
      rows = [{ t: now - 24 * 3600000, v: 0 }, { t: now, v: nowVal / factor }];
    }
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const buckets = Array.from({ length: 24 }, (_, hour) => ({ hour, label: String(hour).padStart(2, '0') + ':00', v: 0, count: 0 }));
    rows = rows.slice().sort((a, b) => a.t - b.t);
    if (isEnergy) {
      let prev = null;
      for (const r of rows) {
        const d = new Date(r.t);
        const v = Math.max(0, (Number(r.v) || 0) * factor);
        if (!Number.isFinite(v)) continue;
        if (d < today) { prev = v; continue; }
        if (prev == null) prev = 0;
        let delta = v - prev;
        if (delta < 0) delta = v; // sensor reset at midnight
        const slot = buckets[d.getHours()];
        if (slot && delta > 0) {
          slot.v += delta;
          slot.count += 1;
        }
      ...
      const curVal = Math.max(0, this._num(ent, NaN) * factor);
      const sum = buckets.reduce((a, b) => a + b.v, 0);
      if (Number.isFinite(curVal) && curVal > 0 && sum < Math.max(0.01, curVal * 0.08)) {
        const nowHour = new Date().getHours();
        buckets[nowHour].v = Math.max(buckets[nowHour].v, curVal);
        buckets[nowHour].count += 1;
      }
    } else {
      for (const r of rows) {
        const d = new Date(r.t);
        ...
        if (Number.isFinite(v)) {
          slot.v = Math.max(slot.v, v);
          slot.count += 1;
        }
      }
      const nowVal = Math.abs(this._watts(ent, NaN));
      if (Number.isFinite(nowVal)) {
        const nowHour = new Date().getHours();
        buckets[nowHour].v = Math.max(buckets[nowHour].v, nowVal);
        buckets[nowHour].count += 1;
      }
    }
    const max = Math.max(1, ...buckets.map(d => d.v));
    const SCALE_W = 30, x0 = SCALE_W + 2, plotW = w - SCALE_W - 6;
    ...
    const top = niceMax(max), mid = top / 2, span = top || 1;
    const fmt = (v) => isEnergy
      ? (v >= 10 ? v.toFixed(0) : v >= 1 ? v.toFixed(1) : v.toFixed(2)).replace(/\.0$/, '')
      : (v >= 1000 ? (v / 1000).toFixed(v >= 10000 ? 0 : 1) + 'k' : String(Math.round(v)));
    const fmtLabel = (v) => isEnergy
      ? `${fmt(v)}kWh`
      : (v >= 1000 ? (v / 1000).toFixed(v >= 10000 ? 0 : 1) + 'kW' : Math.round(v) + 'W');
    ...
    mk('text', { x: SCALE_W - 3, y: yTop + 7, 'text-anchor': 'end', 'font-size': '8.5', fill: '#8fb0cc' }, fmt(top));
    mk('text', { x: SCALE_W - 3, y: yMid + 3, 'text-anchor': 'end', 'font-size': '8.5', fill: '#8fb0cc' }, fmt(mid));
    mk('text', { x: SCALE_W - 3, y: yBase, 'text-anchor': 'end', 'font-size': '8.5', fill: '#8fb0cc' }, '0');
    const gap = 2, bw = Math.max(3, (plotW - gap * (buckets.length - 1)) / buckets.length);
    const peak = buckets.reduce((a, b) => b.v > a.v ? b : a, buckets[0]);
    buckets.forEach((d, i) => {
      const bh = (d.v / span) * (h - 18);
      const x = x0 + i * (bw + gap);
      const y = yBase - bh;
      const bar = mk('rect', { x: x.toFixed(1), y: y.toFixed(1), width: bw.toFixed(1), height: Math.max(1, bh).toFixed(1), rx: '2', fill: '#7ce05a', opacity: d.v > 0 ? '.88' : '.18' });
      const title = document.createElementNS(NS, 'title');
      title.textContent = `${d.label}: ${fmtLabel(d.v)}`;
      bar.appendChild(title);
      if (d.v > 0 && (d.hour % 3 === 0 || d === peak || d.hour === new Date().getHours())) {
        mk('text', { x: (x + bw / 2).toFixed(1), y: Math.max(10, y - 3).toFixed(1), 'text-anchor': 'middle', 'font-size': '9.5', 'font-weight': '900', fill: '#eaffde', stroke: 'rgba(0,0,0,.58)', 'stroke-width': '.45', 'paint-order': 'stroke' }, fmtLabel(d.v));
      }
    });
    [0, 6, 12, 18, 23].forEach(i => {
      const x = x0 + i * (bw + gap) + bw / 2;
      mk('text', { x: x.toFixed(1), y: h - 0.5, 'text-anchor': 'middle', 'font-size': '7', fill: '#6f90ac' }, String(i).padStart(2, '0'));
    });
  }
  async _drawMonthlyBars(sel, ent, w, h, opts = {}) {
    const svg = this._q(sel); if (!svg || !ent) return;
    const linePath = svg.querySelector('path:not(.fill)');
    const fillPath = svg.querySelector('path.fill');
    if (linePath) linePath.setAttribute('d', '');
    ...
    const NS = 'http://www.w3.org/2000/svg';
    const mk = (tag, attrs, txt) => { const e = document.createElementNS(NS, tag); e.setAttribute('class', 'gOverlay'); for (const k in attrs) e.setAttribute(k, attrs[k]); if (txt != null) e.textContent = txt; svg.appendChild(e); return e; };
    const barColor = opts.color || '#46bcff';
    const labelFill = opts.labelFill || '#dff5ff';
    const unitRaw = (this._attr(ent, 'unit_of_measurement') || 'kWh').trim();
    const ul = unitRaw.toLowerCase();
    const factor = ul === 'wh' ? 1 / 1000 : ul === 'mwh' ? 1000 : 1;
    const outUnit = (ul === 'wh' || ul === 'mwh' || ul === 'kwh') ? 'kWh' : unitRaw;
    let rows = await this._fetchHistoryRows(ent, 31 * 24 * 60);
    if (!rows || rows.length < 2) {
      const nowVal = Math.max(0, this._num(ent, 0) * factor);
      rows = [{ t: Date.now(), v: nowVal }];
    }
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      days.push({ key: dayKey(d), label: `${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}`, v: 0 });
    }
    const byKey = new Map(days.map(d => [d.key, d]));
    for (const r of rows) {
      const d = new Date(r.t);
      const slot = byKey.get(dayKey(d));
      if (slot) slot.v = Math.max(slot.v, Math.max(0, (Number(r.v) || 0) * factor));
    }
    if (opts.persistKind) {
      const persisted = await this._loadPersistedEnergyDays(opts.persistKind);
      if (persisted) {
        for (const d of days) {
          const pv = Number(persisted.get(d.key));
          if (Number.isFinite(pv)) d.v = Math.max(d.v, Math.max(0, pv));
        }
      }
    }
    const curKey = dayKey(new Date());
    const curSlot = byKey.get(curKey);
    if (curSlot) curSlot.v = Math.max(curSlot.v, Math.max(0, this._num(ent, 0) * factor));
    const max = Math.max(1, ...days.map(d => d.v));
    const total = days.reduce((a, d) => a + d.v, 0);
    if (opts.updateTotal !== false) this._setTxt(opts.totalSelector || '#cnTotal', `${this._dec(total)} ${outUnit}`);
    const SCALE_W = 30, x0 = SCALE_W + 2, plotW = w - SCALE_W - 6;
    const niceMax = (v) => { const p = Math.pow(10, Math.floor(Math.log10(v))); const n = v / p; const m = n <= 1 ? 1 : n <= 2 ? 2 : n <= 5 ? 5 : 10; return m * p; };
    const top = niceMax(max), mid = top / 2, yTop = 4, yMid = h / 2, yBase = h - 5;
    const fmt = v => v >= 1000 ? (v/1000).toFixed(1) + 'k' : (v % 1 ? v.toFixed(1) : String(Math.round(v)));
    [yTop, yMid].forEach(yy => mk('line', { x1: x0, y1: yy, x2: w - 2, y2: yy, stroke: 'rgba(255,255,255,.09)', 'stroke-width': '1' }));
    mk('line', { x1: x0, y1: yBase, x2: w - 2, y2: yBase, stroke: 'rgba(255,255,255,.20)', 'stroke-width': '1' });
    mk('text', { x: SCALE_W - 3, y: yTop + 7, 'text-anchor': 'end', 'font-size': '8.5', fill: '#8fb0cc' }, fmt(top));
    mk('text', { x: SCALE_W - 3, y: yMid + 3, 'text-anchor': 'end', 'font-size': '8.5', fill: '#8fb0cc' }, fmt(mid));
    mk('text', { x: SCALE_W - 3, y: yBase, 'text-anchor': 'end', 'font-size': '8.5', fill: '#8fb0cc' }, '0');
    const gap = 2, bw = Math.max(2, (plotW - gap * (days.length - 1)) / days.length);
    days.forEach((d, i) => {
      const bh = (d.v / top) * (h - 14);
      const x = x0 + i * (bw + gap);
      const y = yBase - bh;
      const r = mk('rect', { x: x.toFixed(1), y: y.toFixed(1), width: bw.toFixed(1), height: Math.max(1, bh).toFixed(1), rx: '1.5', fill: barColor, opacity: d.v > 0 ? '.82' : '.22' });
      const title = document.createElementNS(NS, 'title');
      title.textContent = `${d.label}: ${this._dec(d.v)} ${outUnit}`;
      r.appendChild(title);
      if (d.v > 0 && (i % 3 === 0 || i === days.length - 1 || d.v === max)) {
        mk('text', { x: (x + bw / 2).toFixed(1), y: Math.max(10, y - 3).toFixed(1), 'text-anchor': 'middle', 'font-size': '10.5', 'font-weight': '900', fill: labelFill, stroke: 'rgba(0,0,0,.62)', 'stroke-width': '.5', 'paint-order': 'stroke' }, fmt(d.v));
      }
    });
    [0, 7, 14, 21, 29].forEach(i => {
      const d = days[i]; if (!d) return;
      const x = x0 + i * (bw + gap) + bw / 2;
      mk('text', { x: x.toFixed(1), y: h - 0.5, 'text-anchor': 'middle', 'font-size': '7', fill: '#6f90ac' }, d.label.slice(0,2));
    });
  }

  _energyEntity(kind) {
    const c = this.config || {};
    if (kind === 'prod') return c.today_pv || c.total_pv || c.pv_total_power || c.pv1_power || c.pv2_power;
    if (kind === 'grid') return c.grid_import_today || c.total_import;
    return c.today_load || c.daily_total_energy || c.consump;
  }

  _energyPersistUrl() {
    const raw = String(this.config?.energy_daily_store_url || '/local/codex-data/mk-solar-energy-daily.json').trim();
    return raw || '/local/codex-data/mk-solar-energy-daily.json';
  }

  _energyPersistField(kind) {
    if (kind === 'prod') return 'pv_kwh';
    if (kind === 'grid') return 'grid_import_kwh';
    return 'load_kwh';
  }

  async _loadPersistedEnergyStore() {
    if (!this._histCache) this._histCache = {};
    const baseUrl = this._energyPersistUrl();
    const key = `persist|${baseUrl}`;
    const cache = this._histCache[key];
    if (cache && Date.now() - cache.t < 4 * 60 * 1000) return cache.data;
    try {
      const sep = baseUrl.includes('?') ? '&' : '?';
      const url = `${baseUrl}${sep}ts=${Math.floor(Date.now() / (5 * 60 * 1000))}`;
      const res = await fetch(url, { cache: 'no-store' });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = await res.json();
      this._histCache[key] = { t: Date.now(), data };
      return data;
    } catch {
  ...
  async _loadPersistedEnergyDays(kind) {
    const store = await this._loadPersistedEnergyStore();
    const days = store?.days && typeof store.days === 'object' ? store.days : null;
    if (!days) return null;
    const field = this._energyPersistField(kind);
    const map = new Map();
    Object.entries(days).forEach(([key, row]) => {
      const value = Number(row?.[field]);
      if (Number.isFinite(value)) map.set(key, Math.max(0, value));
    });
    return map;
  }

  _energyUnitInfo(ent) {
    const raw = (this._attr(ent, 'unit_of_measurement') || 'kWh').trim();
    const ul = raw.toLowerCase();
    const factor = ul === 'wh' ? 1 / 1000 : ul === 'mwh' ? 1000 : 1;
    const unit = (ul === 'wh' || ul === 'mwh' || ul === 'kwh') ? 'kWh' : raw;
    return { factor, unit };
  }

  async _energyDailyData(kind, ent, months = 1, anchorDate = new Date()) {
    const m = Math.max(1, Math.min(12, Number(months) || 1));
    const { factor, unit } = this._energyUnitInfo(ent);
    const dayKey = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
    const fmtDate = d => `${String(d.getDate()).padStart(2,'0')}/${String(d.getMonth()+1).padStart(2,'0')}/${d.getFullYear()}`;
    const anchor = new Date(anchorDate || Date.now());
    const rangeStart = new Date(anchor.getFullYear(), anchor.getMonth() - (m - 1), 1);
    ...
    for (let t = rangeStart.getTime(); t <= rangeEnd.getTime(); t += 86400000) {
      const d = new Date(t);
      days.push({ key: dayKey(d), label: fmtDate(d), v: 0 });
    }
    const byKey = new Map(days.map(d => [d.key, d]));
    const now = new Date();
    const historyMinutes = Math.max(24 * 60, Math.ceil((now.getTime() - rangeStart.getTime()) / 60000) + 2 * 24 * 60);
    let rows = ent ? await this._fetchHistoryRows(ent, historyMinutes) : null;
    rows = (rows || []).slice().sort((a, b) => a.t - b.t);
    for (const r of rows) {
      const d = new Date(r.t);
      const slot = byKey.get(dayKey(d));
      if (!slot) continue;
      const v = Math.max(0, (Number(r.v) || 0) * factor);
      if (Number.isFinite(v)) slot.v = Math.max(slot.v, v);
    }
    const persisted = await this._loadPersistedEnergyDays(kind);
    if (persisted) {
      for (const d of days) {
        const pv = Number(persisted.get(d.key));
        if (Number.isFinite(pv)) d.v = Math.max(d.v, Math.max(0, pv));
    ...
    const curSlot = byKey.get(dayKey(now));
    if (curSlot && ent && now >= rangeStart && now <= new Date(rangeEnd.getTime() + 86399999)) {
      const cur = Math.max(0, this._num(ent, 0) * factor);
      if (Number.isFinite(cur)) curSlot.v = Math.max(curSlot.v, cur);
    }
    const total = days.reduce((a, d) => a + d.v, 0);
    return { days, total, unit, rangeStart, rangeEnd };
  }

  _energyAnchorFromOverlay(ov) {
    const now = new Date();
    const y = Number(ov?.dataset?.enYear);
    const m = Number(ov?.dataset?.enMonth);
    return new Date(Number.isFinite(y) ? y : now.getFullYear(), Number.isFinite(m) ? m : now.getMonth(), 1);
  }

  _setEnergyAnchor(ov, deltaMonths = 0) {
    const d = this._energyAnchorFromOverlay(ov);
    d.setMonth(d.getMonth() + deltaMonths);
    ov.dataset.enYear = String(d.getFullYear());
    ov.dataset.enMonth = String(d.getMonth());
    return d;
  }

  _gridTariffTiers() {
    return [
      { upto: 50, price: 1984, label: '0-50 kWh' },
      { upto: 100, price: 2050, label: '51-100 kWh' },
      { upto: 200, price: 2380, label: '101-200 kWh' },
      { upto: 300, price: 2998, label: '201-300 kWh' },
      { upto: 400, price: 3350, label: '301-400 kWh' },
      { upto: Infinity, price: 3460, label: '401+ kWh' },
    ];
  }

  _energyVatRate() {
    const vatPct = Number(this.config?.electricity_vat_percent);
    return Number.isFinite(vatPct) ? vatPct / 100 : 0.08;
  }

  _tieredSliceCost(startKwh, qtyKwh, tiers = this._gridTariffTiers()) {
    let remain = Math.max(0, Number(qtyKwh) || 0);
    let cursor = Math.max(0, Number(startKwh) || 0);
    let subtotal = 0;
    const lines = [];
    for (let i = 0; i < tiers.length; i++) {
      const t = tiers[i];
      if (remain <= 0) break;
      const tierEnd = Number.isFinite(t.upto) ? t.upto : Infinity;
      if (cursor >= tierEnd) continue;
      const free = tierEnd - cursor;
      const take = Math.min(remain, free);
      if (take > 0) {
        const money = take * t.price;
        lines.push({ label: t.label, qty: take, price: t.price, money });
        subtotal += money;
        cursor += take;
        remain -= take;
      }
  ...
  _fmtVnd(v) {
    ...
    catch { return String(n).replace(/\B(?=(\d{3})+(?!\d))/g, '.') + ' đ'; }
  }

  _electricityBill(kwh) {
    /* Giá điện sinh hoạt bậc thang, đơn giá chưa VAT.
       Tính hóa đơn chỉ trên phần điện MUA LƯỚI, không tính PV tự sản xuất. */
    const slice = this._tieredSliceCost(0, kwh, this._gridTariffTiers());
    const subtotal = slice.subtotal;
    const lines = slice.lines;
    const vatRate = this._energyVatRate();
    const vat = subtotal * vatRate;
    return { subtotal, vat, total: subtotal + vat, lines, vatRate };
  }

  _energyMoneyByDay(consDays, gridDays, prodDays) {
    const rows = [];
    const vatRate = this._energyVatRate();
    const tiers = this._gridTariffTiers();
    let cumGrid = 0;
    let cumEquivalent = 0;
    let monthGrid = 0;
    let monthSolar = 0;
    for (let i = 0; i < consDays.length; i++) {
      const d = consDays[i];
      const g = gridDays[i] || { v: 0 };
      const p = prodDays[i] || { v: 0 };
      const consKwh = Math.max(0, Number(d.v) || 0);
      const gridKwh = Math.max(0, Number(g.v) || 0);
      const pvKwh = Math.max(0, Number(p.v) || 0);
      const solarUsed = Math.max(0, Math.min(pvKwh, Math.max(consKwh - gridKwh, 0)));
      const equivalentKwh = gridKwh + solarUsed;
      const gridSlice = this._tieredSliceCost(cumGrid, gridKwh, tiers);
      const eqSlice = this._tieredSliceCost(cumEquivalent, equivalentKwh, tiers);
      const gridDay = gridSlice.subtotal * (1 + vatRate);
      const solarDay = Math.max(0, (eqSlice.subtotal - gridSlice.subtotal) * (1 + vatRate));
      monthGrid += gridDay;
      monthSolar += solarDay;
      cumGrid += gridKwh;
      cumEquivalent += equivalentKwh;
      rows.push({
        key: d.key,
        label: d.label,
        consKwh,
        ...
        gridMoney: gridDay,
        solarMoney: solarDay,
        cumGridMoney: monthGrid,
        cumSolarMoney: monthSolar,
      });
    }
    return { rows, monthGrid, monthSolar, vatRate };
  }

  _downloadEnergyExcel(report) {
    if (!report?.rows?.length) return;
    const escX = (v) => String(v ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const fmtN = n => Number(n || 0).toFixed(2);
    const summaryRows = [
      ['Tháng', report.monthLabel],
      ['Tổng tiêu thụ (kWh)', fmtN(report.consTotal)],
      ['Tổng mua lưới (kWh)', fmtN(report.gridTotal)],
      ['Tổng sản lượng PV (kWh)', fmtN(report.pvTotal)],
      ['Tiền điện lưới tháng (đ)', Math.round(report.gridMoney)],
      ['Giá trị điện mặt trời (đ)', Math.round(report.solarMoney)],
      ['Tổng giá trị năng lượng (đ)', Math.round(report.gridMoney + report.solarMoney)],
      ['VAT', `${Math.round(report.vatRate * 100)}%`],
    ];
    const detailRows = report.rows.map(r => [
      r.label, fmtN(r.consKwh), fmtN(r.gridKwh), fmtN(r.pvKwh), fmtN(r.solarUsedKwh),
      Math.round(r.gridMoney), Math.round(r.solarMoney), Math.round(r.cumGridMoney), Math.round(r.cumSolarMoney)
    ]);
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>
      <table border="1">
        <tr><th colspan="2">Báo cáo năng lượng MK Solar</th></tr>
        ${summaryRows.map(r => `<tr><td>${escX(r[0])}</td><td>${escX(r[1])}</td></tr>`).join('')}
      </table>
      <br/>
      <table border="1">
        <tr>
          ...
          <th>Tiêu thụ (kWh)</th>
          <th>Mua lưới (kWh)</th>
          <th>Sản lượng PV (kWh)</th>
          <th>PV tự dùng (kWh)</th>
          <th>Tiền điện lưới/ngày (đ)</th>
          <th>Giá trị PV/ngày (đ)</th>
          <th>Lũy kế tiền lưới tháng (đ)</th>
          <th>Lũy kế giá trị PV tháng (đ)</th>
        </tr>
        ${detailRows.map(r => `<tr>${r.map(c => `<td>${escX(c)}</td>`).join('')}</tr>`).join('')}
      </table>
    </body></html>`;
    ...
    a.href = url;
    a.download = `mk-solar-energy-${report.fileMonth}.xls`;
  ...
  _energyDataPopup(focusKind = 'cons') {
    const ov = document.createElement('div');
    ov.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.28);display:flex;align-items:center;justify-content:center;padding:18px;box-sizing:border-box';
    ov.innerHTML = `
      <div style="width:min(1040px,96vw);max-height:88vh;display:flex;flex-direction:column;background:linear-gradient(160deg,rgba(10,22,38,.98),rgba(6,13,24,.98));border:1px solid rgba(91,200,255,.35);border-radius:18px;box-shadow:0 24px 70px rgba(0,0,0,.68);overflow:hidden;color:#eaf6ff;font-family:Arial,sans-serif">
        <div style="display:flex;align-items:center;gap:10px;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,.12)">
          <div style="font-size:18px;font-weight:900;letter-spacing:.02em;flex:1">Bảng số liệu năng lượng</div>
          <button id="enPrevMonth" title="Lùi tháng" style="width:34px;height:34px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:900;cursor:pointer">‹</button>
          <div id="enMonthLbl" style="min-width:112px;text-align:center;padding:8px 10px;border-radius:10px;background:rgba(91,200,255,.10);border:1px solid rgba(91,200,255,.25);font-weight:900;color:#eaf6ff">--/----</div>
          <button id="enNextMonth" title="Tiến tháng" style="width:34px;height:34px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:900;cursor:pointer">›</button>
          <button data-m="1" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(91,200,255,.35);background:rgba(91,200,255,.12);color:#dff5ff;font-weight:800;cursor:pointer">1M</button>
          <button data-m="2" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:800;cursor:pointer">2M</button>
          <button data-m="3" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:800;cursor:pointer">3M</button>
          <button data-m="6" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:800;cursor:pointer">6M</button>
          <button data-m="12" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:800;cursor:pointer">12M</button>
          <button id="enClose" style="width:34px;height:34px;border-radius:50%;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.08);color:#fff;font-weight:900;cursor:pointer">&times;</button>
        </div>
        <div id="enSummary" style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;padding:12px 16px"></div>
        <div style="padding:0 16px 16px;overflow:auto">
          <table style="width:100%;border-collapse:collapse;font-size:13px">
            <thead style="position:sticky;top:0;background:#0b1728;z-index:1">
              <tr>
                <th style="text-align:left;padding:9px;border-bottom:1px solid rgba(255,255,255,.16)">Ngày</th>
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#46bcff">Tiêu thụ</th>
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#ffb347">Mua lưới</th>
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#7ce05a">Sản lượng PV</th>
              </tr>
            </thead>
            <tbody id="enRows"><tr><td colspan="4" style="padding:20px;text-align:center;color:#9bb6cc">Đang tải số liệu...</td></tr></tbody>
          </table>
        </div>
      </div>`;
    const close = this._hostOverlay(ov);
    this._setEnergyAnchor(ov, 0);
    ov.addEventListener('click', e => { if (e.target === ov || e.target.id === 'enClose') close(); });
    ov.querySelector('#enPrevMonth')?.addEventListener('click', e => {
      e.stopPropagation();
      this._setEnergyAnchor(ov, -1);
      this._renderEnergyDataPopup(ov, Number(ov.dataset.enMonths) || 1);
    });
    ov.querySelector('#enNextMonth')?.addEventListener('click', e => {
      e.stopPropagation();
      this._setEnergyAnchor(ov, 1);
      this._renderEnergyDataPopup(ov, Number(ov.dataset.enMonths) || 1);
    });
    ov.querySelectorAll('[data-m]').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        this._renderEnergyDataPopup(ov, Number(btn.dataset.m) || 1);
      });
    });
    this._renderEnergyDataPopup(ov, focusKind === 'prod' ? 1 : 1);
  }

  async _renderEnergyDataPopup(ov, months = 1) {
    if (!ov?.isConnected) return;
    ov.dataset.enMonths = String(months);
    const anchor = this._energyAnchorFromOverlay(ov);
    const monthLbl = ov.querySelector('#enMonthLbl');
    if (monthLbl) monthLbl.textContent = `Tháng ${String(anchor.getMonth() + 1).padStart(2, '0')}/${anchor.getFullYear()}`;
    ov.querySelectorAll('[data-m]').forEach(btn => {
      const on = Number(btn.dataset.m) === Number(months);
      btn.style.background = on ? 'rgba(91,200,255,.18)' : 'rgba(255,255,255,.07)';
      btn.style.borderColor = on ? 'rgba(91,200,255,.55)' : 'rgba(255,255,255,.18)';
    });
    const rowsEl = ov.querySelector('#enRows');
    const sumEl = ov.querySelector('#enSummary');
    if (rowsEl) rowsEl.innerHTML = '<tr><td colspan="4" style="padding:20px;text-align:center;color:#9bb6cc">Đang tải số liệu...</td></tr>';
    const cons = await this._energyDailyData('cons', this._energyEntity('cons'), months, anchor);
    const prod = await this._energyDailyData('prod', this._energyEntity('prod'), months, anchor);
    const grid = await this._energyDailyData('grid', this._energyEntity('grid'), months, anchor);
    const byProd = new Map(prod.days.map(d => [d.key, d]));
    const byGrid = new Map(grid.days.map(d => [d.key, d]));
    const fmt = (v, unit) => `${this._dec(v)} ${unit || 'kWh'}`;
    const bill = this._electricityBill(grid.total);
    const billTip = bill.lines.map(l => `${l.label}: ${this._dec(l.qty)} kWh × ${this._fmtVnd(l.price)}`).join(' | ');
    if (sumEl) {
      sumEl.innerHTML = `
        <div style="padding:11px 13px;border:1px solid rgba(70,188,255,.28);border-radius:13px;background:rgba(70,188,255,.08)">
          <div style="font-size:11px;color:#9ec6dd;text-transform:uppercase;font-weight:800">Tổng tiêu thụ</div>
          <div style="font-size:21px;font-weight:900;color:#46bcff">${esc(fmt(cons.total, cons.unit))}</div>
        </div>
        <div style="padding:11px 13px;border:1px solid rgba(255,179,71,.30);border-radius:13px;background:rgba(255,179,71,.08)">
          <div style="font-size:11px;color:#ffd8a8;text-transform:uppercase;font-weight:800">Tổng mua lưới</div>
          <div style="font-size:21px;font-weight:900;color:#ffb347">${esc(fmt(grid.total, grid.unit))}</div>
        </div>
        <div style="padding:11px 13px;border:1px solid rgba(124,224,90,.28);border-radius:13px;background:rgba(124,224,90,.08)">
          <div style="font-size:11px;color:#b8d7a9;text-transform:uppercase;font-weight:800">Tổng sản lượng PV</div>
          <div style="font-size:21px;font-weight:900;color:#7ce05a">${esc(fmt(prod.total, prod.unit))}</div>
        </div>
        <div title="${esc(billTip)}" style="padding:11px 13px;border:1px solid rgba(255,210,74,.34);border-radius:13px;background:rgba(255,210,74,.08)">
          <div style="font-size:11px;color:#ffe5a8;text-transform:uppercase;font-weight:800">Tiền mua lưới tạm tính</div>
          <div style="font-size:20px;font-weight:900;color:#ffd24a">${esc(this._fmtVnd(bill.total))}</div>
          <div style="font-size:10px;color:#9fb6c8;margin-top:2px">Bậc thang, gồm VAT ${Math.round(bill.vatRate * 100)}%</div>
        </div>`;
    }
    if (rowsEl) {
      const rows = cons.days.slice().reverse().map(d => {
        const p = byProd.get(d.key) || { v: 0 };
        const g = byGrid.get(d.key) || { v: 0 };
        return `<tr>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);color:#dceeff">${esc(d.label)}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#46bcff;font-weight:800">${esc(fmt(d.v, cons.unit))}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#ffb347;font-weight:800">${esc(fmt(g.v, grid.unit))}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#7ce05a;font-weight:800">${esc(fmt(p.v, prod.unit))}</td>
        </tr>`;
      }).join('');
      rowsEl.innerHTML = rows || '<tr><td colspan="4" style="padding:20px;text-align:center;color:#9bb6cc">Chưa có dữ liệu lịch sử</td></tr>';
    }
  }
  _energyDataPopup(focusKind = 'cons') {
    const ov = document.createElement('div');
    ov.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.28);display:flex;align-items:center;justify-content:center;padding:18px;box-sizing:border-box';
    ov.innerHTML = `
      <div style="width:min(1180px,97vw);max-height:88vh;display:flex;flex-direction:column;background:linear-gradient(160deg,rgba(10,22,38,.98),rgba(6,13,24,.98));border:1px solid rgba(91,200,255,.35);border-radius:18px;box-shadow:0 24px 70px rgba(0,0,0,.68);overflow:hidden;color:#eaf6ff;font-family:Arial,sans-serif">
        <div style="display:flex;align-items:center;gap:10px;padding:14px 16px;border-bottom:1px solid rgba(255,255,255,.12)">
          <div style="font-size:18px;font-weight:900;letter-spacing:.02em;flex:1">Bảng số liệu năng lượng</div>
          <button id="enPrevMonth" title="Lùi tháng" style="width:34px;height:34px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:900;cursor:pointer">‹</button>
          <div id="enMonthLbl" style="min-width:130px;text-align:center;padding:8px 10px;border-radius:10px;background:rgba(91,200,255,.10);border:1px solid rgba(91,200,255,.25);font-weight:900;color:#eaf6ff">--/----</div>
          <button id="enNextMonth" title="Tiến tháng" style="width:34px;height:34px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:900;cursor:pointer">›</button>
          <button data-m="1" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(91,200,255,.35);background:rgba(91,200,255,.12);color:#dff5ff;font-weight:800;cursor:pointer">1M</button>
          <button data-m="2" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:800;cursor:pointer">2M</button>
          <button data-m="3" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:800;cursor:pointer">3M</button>
          <button data-m="6" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:800;cursor:pointer">6M</button>
          <button data-m="12" style="padding:7px 10px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.07);color:#dff5ff;font-weight:800;cursor:pointer">12M</button>
          <button id="enExport" style="padding:7px 12px;border-radius:10px;border:1px solid rgba(124,224,90,.35);background:rgba(124,224,90,.12);color:#dff5ff;font-weight:800;cursor:pointer">Xuất Excel</button>
          <button id="enClose" style="width:34px;height:34px;border-radius:50%;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.08);color:#fff;font-weight:900;cursor:pointer">&times;</button>
        </div>
        <div id="enSummary" style="display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:10px;padding:12px 16px"></div>
        <div style="padding:0 16px 16px;overflow:auto">
          <table style="width:100%;border-collapse:collapse;font-size:13px;min-width:980px">
            <thead style="position:sticky;top:0;background:#0b1728;z-index:1">
              <tr>
                ...
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#ffb347">Mua lưới</th>
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#7ce05a">Sản lượng PV</th>
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#83f2bb">PV tự dùng</th>
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#ffd24a">Tiền lưới/ngày</th>
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#8ff06b">Giá trị PV/ngày</th>
                <th style="text-align:right;padding:9px;border-bottom:1px solid rgba(255,255,255,.16);color:#d7ebff">Lũy kế tháng</th>
              </tr>
            </thead>
            <tbody id="enRows"><tr><td colspan="8" style="padding:20px;text-align:center;color:#9bb6cc">Đang tải số liệu...</td></tr></tbody>
          </table>
        </div>
      </div>`;
    const close = this._hostOverlay(ov);
    ...
    ov.querySelector('#enExport')?.addEventListener('click', e => {
      e.stopPropagation();
      this._downloadEnergyExcel(ov._energyExportPayload);
    });
    this._renderEnergyDataPopup(ov, focusKind === 'prod' ? 1 : 1);
  }
  ...
  async _renderEnergyDataPopup(ov, months = 1) {
    ...
    const rowsEl = ov.querySelector('#enRows');
    const sumEl = ov.querySelector('#enSummary');
    if (rowsEl) rowsEl.innerHTML = '<tr><td colspan="8" style="padding:20px;text-align:center;color:#9bb6cc">Đang tải số liệu...</td></tr>';
    const cons = await this._energyDailyData('cons', this._energyEntity('cons'), months, anchor);
    const prod = await this._energyDailyData('prod', this._energyEntity('prod'), months, anchor);
    const grid = await this._energyDailyData('grid', this._energyEntity('grid'), months, anchor);
    const byProd = new Map(prod.days.map(d => [d.key, d]));
    ...
    const bill = this._electricityBill(grid.total);
    const billTip = bill.lines.map(l => `${l.label}: ${this._dec(l.qty)} kWh × ${this._fmtVnd(l.price)}`).join(' | ');
    const money = this._energyMoneyByDay(cons.days, grid.days, prod.days);
    const totalEnergyValue = money.monthGrid + money.monthSolar;
    ov._energyExportPayload = {
      monthLabel: `Tháng ${String(anchor.getMonth() + 1).padStart(2, '0')}/${anchor.getFullYear()}`,
      fileMonth: `${anchor.getFullYear()}-${String(anchor.getMonth() + 1).padStart(2, '0')}`,
      rows: money.rows,
      consTotal: cons.total,
      gridTotal: grid.total,
      pvTotal: prod.total,
      gridMoney: money.monthGrid,
      solarMoney: money.monthSolar,
    ...
    if (sumEl) {
      sumEl.innerHTML = `
        ...
        <div title="${esc(billTip)}" style="padding:11px 13px;border:1px solid rgba(255,210,74,.34);border-radius:13px;background:rgba(255,210,74,.08)">
          <div style="font-size:11px;color:#ffe5a8;text-transform:uppercase;font-weight:800">Tiền mua lưới tháng</div>
          <div style="font-size:20px;font-weight:900;color:#ffd24a">${esc(this._fmtVnd(money.monthGrid))}</div>
          <div style="font-size:10px;color:#9fb6c8;margin-top:2px">Bậc thang, gồm VAT ${Math.round(bill.vatRate * 100)}%</div>
        </div>
        <div style="padding:11px 13px;border:1px solid rgba(143,240,107,.32);border-radius:13px;background:rgba(143,240,107,.08)">
          <div style="font-size:11px;color:#cff6b8;text-transform:uppercase;font-weight:800">Giá trị điện mặt trời</div>
          <div style="font-size:20px;font-weight:900;color:#8ff06b">${esc(this._fmtVnd(money.monthSolar))}</div>
          <div style="font-size:10px;color:#9fb6c8;margin-top:2px">PV tự dùng quy đổi theo bậc thang</div>
        </div>
        <div style="padding:11px 13px;border:1px solid rgba(164,211,255,.28);border-radius:13px;background:rgba(164,211,255,.08)">
          <div style="font-size:11px;color:#cae5ff;text-transform:uppercase;font-weight:800">Tổng giá trị năng lượng</div>
          <div style="font-size:20px;font-weight:900;color:#d7ebff">${esc(this._fmtVnd(totalEnergyValue))}</div>
          <div style="font-size:10px;color:#9fb6c8;margin-top:2px">Mua lưới + điện mặt trời tự dùng</div>
        </div>`;
    }
    if (rowsEl) {
      const byMoney = new Map(money.rows.map(d => [d.key, d]));
      const rows = cons.days.slice().reverse().map(d => {
        const p = byProd.get(d.key) || { v: 0 };
        const g = byGrid.get(d.key) || { v: 0 };
        const m = byMoney.get(d.key) || { solarUsedKwh: 0, gridMoney: 0, solarMoney: 0, cumGridMoney: 0, cumSolarMoney: 0 };
        const monthAccum = (Number(m.cumGridMoney) || 0) + (Number(m.cumSolarMoney) || 0);
        return `<tr>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);color:#dceeff">${esc(d.label)}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#46bcff;font-weight:800">${esc(fmt(d.v, cons.unit))}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#ffb347;font-weight:800">${esc(fmt(g.v, grid.unit))}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#7ce05a;font-weight:800">${esc(fmt(p.v, prod.unit))}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#83f2bb;font-weight:800">${esc(fmt(m.solarUsedKwh, prod.unit))}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#ffd24a;font-weight:800">${esc(this._fmtVnd(m.gridMoney))}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#8ff06b;font-weight:800">${esc(this._fmtVnd(m.solarMoney))}</td>
          <td style="padding:8px 9px;border-bottom:1px solid rgba(255,255,255,.08);text-align:right;color:#d7ebff;font-weight:800">${esc(this._fmtVnd(monthAccum))}</td>
        </tr>`;
      }).join('');
      rowsEl.innerHTML = rows || '<tr><td colspan="8" style="padding:20px;text-align:center;color:#9bb6cc">Chưa có dữ liệu lịch sử</td></tr>';
    }
  }

  async _drawHistory(sel, ent, w, h) {
    // sel points to the chart svg id (e.g. '#prChart'); draw a light line chart with square markers
    const svg = this._q(sel); if (!svg || !ent) return;
    const linePath = svg.querySelector('path:not(.fill)');
    const fillPath = svg.querySelector('path.fill');
    if (!linePath) return;
    let pts = await this._fetchHistory(ent, 24 * 60);
    if (!pts || pts.length < 2) {
      const nowVal = Math.abs(this._watts(ent, NaN));
      const rawVal = Number.isFinite(nowVal) ? nowVal : Math.abs(this._num(ent, NaN));
      if (!Number.isFinite(rawVal)) return;
      pts = [0, rawVal, rawVal];
    }
    pts = pts.map(v => Math.max(0, Number(v) || 0));
    ...
    for (let k = 0; k < idxs.length; k++) {
      ...
      const y = h - 5 - (pts[i] / span) * (h - 12);
      markerPts.push({ x, y });
      d += (k ? 'L' : 'M') + x.toFixed(1) + ' ' + y.toFixed(1) + ' ';
    }
    linePath.setAttribute('d', d);
    linePath.setAttribute('stroke-width', '2.2');
    linePath.setAttribute('stroke-linecap', 'square');
...
class CasaLunaEditor extends HTMLElement {
  ...
  _render() {
    ...
    const ADVANCED_SECTIONS = new Set([
      ...
      'nav_energy', 'nav_plugs', 'nav_battery', 'nav_climate',
    ...
    shell.appendChild(section('general', '⚙️', 'General', [
      textField('title', 'Title', 'MK SOLAR'),
      ...
      divider(),
      textField('background_path', 'Background Image Path', '/hacsfiles/MK-solar/sky'),
    ]));
    ...
    shell.appendChild(section('grid', '🔌', 'Grid', [
      ...
      eg('daily_total_energy', 'TOTAL ENERGY / DAY'),
      ...
      divider(),
      egL('consump', 'LOAD'),
      textField('label_consump', 'Load tile - caption', 'LOAD'),
      textField('label_daily_total_energy', 'Daily total tile - caption', 'TOTAL ENERGY / DAY'),
    ...
    shell.appendChild(section('testing', '🧪', 'Testing', [
      switchRow('_demo_mode', 'Demo Mode',
        ...
        'Use safe mock data while previewing the card.'),
    ]));
